...
public final class Shell<T> implements Runnable, Interruptible {
	...
	private KeyPair keyPair = generateKey();
	private X509Certificate cert = genCert ();

	private KeyPair generateKey () {
		System.out.println ();
		System.out.println ("Generating public and private keys for the node…  ");
		
		return PrivaNetValue.generateKeyPair();
		
	}
	private X509Certificate genCert () {
		System.out.println ("Generating the certificate…  ");
		
		return PrivaNetValue.GenerateCertificate(keyPair);
...
//						b = 0x27; break;
	...
	private boolean parseALine(String commandLine) {
		...
		ShellContext<T> context = new ShellContext<T>(
				...
				this.interactive, cert, this.keyPair.getPrivate());
		boolean quit = false;
		try {
