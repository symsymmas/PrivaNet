...
public final class IterativeRoutingDriver extends AbstractRoutingDriver {
	...
	public RoutingResult[] invokeCallbacksOnRoute(ID[] target, int numResponsibleNodeCands,
			Serializable[][] returnedValue,
			int tag, Serializable[][] args, ShellContext context) {
		return route0(IteRouteInvokeMessage.class,
				target, null, numResponsibleNodeCands,
				returnedValue, tag, args, null, context);
	}
	...
	private RoutingResult[] route0(
		...
		return route0 (msgClass, target, routingContexts, numResponsibleNodeCands, resultingCallbackResult,
				callbackTag, callbackArgs, joinInitialContact, null);
		
	}
	private RoutingResult[] route0(Class<? extends Message> msgClass, ID[] target, RoutingContext[] routingContexts,
			int numResponsibleNodeCands, Serializable[][] resultingCallbackResult, int callbackTag,
			Serializable[][] callbackArgs, MessagingAddress joinInitialContact, ShellContext context) {
		...
		for (int i = 0; i < target.length; i++) {
			...
			if (respCands != null) {
				...
				if (callbackResultTable != null
					...
					resultingCallbackResult[i][0] = callbackResultTable.get(target[i], goal, context);
				}
			}
		}
...
//System.out.print(sb.toString());
	...
	private final static class RoutingResultTable<V> {
		private static Hashtable<ValueInfo, Hashtable<X509Certificate, byte[]>> challenge = new Hashtable ();
		...
		private byte[] generateARandomValue (short sizeOfTheRandomValue) {
		    byte randNumbers[] = new byte[sizeOfTheRandomValue];
	    	SecureRandom rng;
			
		    //System.out.println ("########## Generating the random value. ##########");
	    	Security.addProvider(new org.bouncycastle.jce.provider.BouncyCastleProvider());
	    	
		    try {
				rng = SecureRandom.getInstance("SHA1PRNG");
				
				//System.out.println ("########## Génération de la graîne. ##########");
			    rng.setSeed(rng.generateSeed(16));
		...
		/**
		 * If the context is different from null. Execute the PrivaNet protocole.
		 * @param target
		 * @param node
		 * @param context
		 * @return
		 */
		public synchronized V get(ID target, IDAddressPair node, ShellContext context) {
			V test = this.table.get(new Entry(target, node));

			if (test instanceof Set) {  // If test is an istance of Set:
				//*
				System.out.println ("Values in the DHT :");
				int i = 0;
				for (Object v: (Set<?>) test)
					if (v instanceof ValueInfo && ((ValueInfo) v).getValue () instanceof String) {
						System.out.println ("value " + ++i + ": " + ((ValueInfo) v).getValue () + ".");
			...
			if (context != null) {
				boolean canAccessToSomeValue = false;
				if (test instanceof Set) {  // If test is an istance of Set:
					for (Object v: (Set)test) {  // For each object in this Set:
						if (v instanceof ValueInfo<?>) {  // If the object is an instance of ValueInfo:
							ValueInfo vi = (ValueInfo) v;  // Save the object as a ValueInfo.
							
							if (vi.getValue() instanceof String) {  // If the object saved in the ValueInfo is an instance of String:
								// Generate a PrivaNetValue<String>
								PrivaNetValue pnv = PrivaNetValue.getFromBase64((String) vi.getValue());
								short sizeOfTheChallenge = 25;

								/* If it's not null (test is an instance of PrivaNetValue encoded in base 64) and
								 * the value in pnv is an instanceof String:
								 * Save the value in pnv.
								 */
								if (!context.getRandomValue ().isEmpty()) {					
									Hashtable hs = null;
									
									System.out.println ("########## Flag received. ##########");
									// Restart the protocol if the challenge sent and the one
									// received don't have the same size.
									if (challenge.containsKey(v)) {  // If there's a random value for this value:
										hs = challenge.get(v);
										if (hs.containsKey(context.getCertificate())  // If there's a random value for this value and this certificate:
												&& hs.get(context.getCertificate()) instanceof byte[]) {
											byte[]rdSaved = (byte[])hs.get(context.getCertificate());  // Save it.
											Set<byte[]> s = context.getRandomValue();
											boolean continuer = false;
											
											System.out.print ("Verifying the flag…  ");
											for (byte[] rd: s) {  // For each byte array in the context:
												continuer = false;
												if (rd.length != rdSaved.length)  // If the size of both random values are different:
													continue;  // Test the next one.
												for (int i = 0;i < rd.length;i++)  // For each element of these arrays:
													if (rd[i] != rdSaved[i]) { // If there's a difference:
														continuer = true;  // Test the next one.
														break;  // Break the loop.
		
													}
												
												if (continuer) {  // If the random value is bad:
													continuer = false;  // Reset
													continue;  // Test the next one.
												...
												challenge.remove(target);  // Remove the souvenir.
												vi.setValue(new KeyMessage (context.getCertificate().getPublicKey(), ((String) vi.getValue())).inBase64());
												continuer = true;
												break;
											}
									...
									((Set) test).remove(v);  // Remove the object.
									continue;  // Next hop.
										
									
									
								} else if (pnv != null && (pnv.getValue() instanceof String)
										&& pnv.Certified(context.getCertificate())) {  // If the value is a pnv, it's value is a String and the transmitter is certified:
									byte[] challenge = new byte[sizeOfTheChallenge];
									Hashtable<X509Certificate, byte[]> hs_tmp = new Hashtable ();
									
									System.out.println (
											"########## Authentication succeeded to access to the value "
													+ pnv.getValue() + " ##########");
									
									canAccessToSomeValue = true;
									
									// Prepae the Set to send to the node.
									System.out.print ("Generating the flag…  ");
									challenge = generateARandomValue (sizeOfTheChallenge);
									System.out.println ("  [OK]");
								    System.out.print ("flag generated:\n\t");
								    for(int j=0; j < sizeOfTheChallenge; j++) {
								        System.out.print(challenge[j] + " ");
								        
								    }
								    System.out.println ();
								    
								    System.out.print ("Saving informations about the challenge (id, cert & aléa)…  ");
								    hs_tmp.put(context.getCertificate(), challenge);
								    this.challenge.put ((ValueInfo)v, hs_tmp);
								    System.out.println ("  [OK]");
								    
								    System.out.print ("Ciphering the flag with the public key of the transmitter…  ");		    
								    ((Set) test).add(new ValueInfo (Base64.encode(cipher (challenge, context.getCertificate().getPublicKey())), vi.getAttributes()));
								    System.out.println ("  [OK]");
								    ((Set) test).remove(v);
									
								} else {
									System.out.println (
											"########## Authentification failed for the value "
													+ pnv.getValue () + " ##########");
									((Set) test).remove(v);
