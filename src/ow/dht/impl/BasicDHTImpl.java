...
public class BasicDHTImpl<V extends Serializable> implements DHT<V> {
	...
	/**
	 * Get remotely verifying the certificate.
	 * @param keys
	 * @param results
	 * @param context
	 * @return
	 */
	protected RoutingResult[] getRemotely(ID[] keys, Set<ValueInfo<V>>[] results, ShellContext context) {
		...
		RoutingResult[] routingRes = null;
		
		if (context == null)
			 routingRes = this.routingSvc.invokeCallbacksOnRoute(
		...
		//*
			 ...
			 routingRes = this.routingSvc.invokeCallbacksOnRoute(
						keys, config.getNumTimesGets() + config.getNumSpareResponsibleNodeCandidates(),
						callbackResultContainer, -1, args, context);
						//*/
	...
	/**
	 * Get remotely, without certificate.
	 * @param keys
	 * @param results
	 * @return
	 */
	protected RoutingResult[] getRemotely(ID[] keys, Set<ValueInfo<V>>[] results) {
		return getRemotely (keys, results, null);
...
//System.out.println("  value: " + v);
	...
	@Override
	public Set<ValueInfo<V>>[] get(ID[] keys, ShellContext context) {
